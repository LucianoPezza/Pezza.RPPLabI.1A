...
int altaTrabajos(int idx, eTrabajo listaT[], int tam,int tamt, eNotebook listaN[], eServicio listaS[], eMarca listaM[], eTipo listaTI[],eCliente listaC[], eVendedor listaVE[])
{
    ...
    {
        mostrarNotebooks(listaN, tam, listaM, listaTI,tamt, listaC, listaVE );
...
void mostrarNotebookConFecha(eNotebook lista, eMarca listaM[],eTipo listaT[], int tammaster,eCliente listaC[], eTrabajo listaTR)
{
    ...
    printf("\n%d %10s %10s %13s %10s %10d %10d/%d/%d",lista.id, lista.modelo, Marcades, Tipodes,nombreClientes, lista.precio, listaTR.unaFecha.dia, listaTR.unaFecha.mes, listaTR.unaFecha.anio);
}
